...
$conn = new mysqli('localhost', 'root', '','incidencias');
 if ($conn->connect_error) {
    ...
    $sql = "SELECT * FROM `incidencia` join`estado` ON `incidencia`.`estado_id`=`estado`.`id` where `reportador_usuario_id` ='$user' and `incidencia`.`estado_id`<> 6";
...
//echo '<pre>' . var_export($fila, true) . '</pre>';
    ...
    if (isset($_POST['close_incidencia'])) {
            ...
            if (isset($_POST['search_id'])) {
                if (!empty($_POST['search_id_number'])) {
                    ...
                    $res_inc=$conn->query($sql_1);
                        ...
                        echo "<script type=\"text/javascript\">alert(\"El id insertado no existe o ya fue cerrada\");</script>";  
                    }   
                }else{
        ...
        <?php 
            ...
            $id = (int)$_POST['id_usu'];
            $conn = new mysqli('localhost', 'root', '','incidencias');
                $sql ="UPDATE `incidencia` SET `estado_id`=6 WHERE`id`=$id";
                        $Res=$conn->query($sql);
