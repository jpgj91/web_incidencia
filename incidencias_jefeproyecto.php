...
$conn = new mysqli('localhost', 'root', '','incidencias');
 if ($conn->connect_error) {
    ...
    $sql = "SELECT `incidencia`.`id`,`incidencia`.`asunto`,`prioridad`.`name`,`estado`.`name`,`usuario`.`name`,`error`.`name`,`incidencia`.`fecha` FROM `incidencia` join`estado` ON `incidencia`.`estado_id`=`estado`.`id` join`prioridad` ON `incidencia`.`prioridad_id`=`prioridad`.`id` join`error` ON `incidencia`.`error_id`=`error`.`id`  left join`usuario` ON `incidencia`.`asignado_usuario_id`=`usuario`.`id` Order by `incidencia`.`estado_id`";
    ...
    $conn->close();// Establecemos conexion con BD y creamos la query para que nos devuelva  lo que necesitemos  y la cerramos 
                   ...
                   echo "<table id='incidecnias_general'>
                            ...
                            <tr>
                             ...
                             <tr>
                                ...
                                <th id='t_assingacion'>Asignado</th>
